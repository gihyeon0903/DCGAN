...
class CelebA_Datasets(Dataset):
      ...
      def data_extract_from_csv(self, data_type, celebA_attr):
            if data_type == 'SmileMan':
                  filtered_data = celebA_attr.loc[(celebA_attr.Smiling == 1) & (celebA_attr.Male == 1), ]
            elif data_type == 'NonSmileMan':
                  filtered_data = celebA_attr.loc[(celebA_attr.Smiling == -1) & (celebA_attr.Male == 1), ]
            elif data_type == 'NonSmileWoman':
                  filtered_data = celebA_attr.loc[(celebA_attr.Smiling == -1) & (celebA_attr.Male == -1), ]
